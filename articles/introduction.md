...
R. The Nomis API is free to access without registration, and contains
up-to-date official statistics, including data from the Census, the
Labour Force Survey, DWP benefit statistics and other economic and
demographic data. Nomis is maintained on behalf of the Office for
National Statistics by the University of Durham.
...
Nomis data is based around administrative and statistical geographies,
and a particular geography should be specified when downloading data.

`nomisr` is designed around a pipeline of three key functions:
...
The
[`nomis_data_info()`](https://docs.ropensci.org/nomisr/reference/nomis_data_info.md)
function is focused on the structure and coverage of the available
datasets.

Use the
[`nomis_data_info()`](https://docs.ropensci.org/nomisr/reference/nomis_data_info.md)
function without any parameters to get a tibble with metadata for all
available datasets:
...
[`nomis_data_info()`](https://docs.ropensci.org/nomisr/reference/nomis_data_info.md)
can also be used to query metadata from a specific dataset, using its
ID. The example below uses the “LC4408EW - Tenure by number of persons
per bedroom in household by household type” dataset from the 2011
census, which has the ID “NM_893_1”.
...
`components.attribute` and `components.dimension`, are list-columns of
...
detailed status metadata. `components.dimension` contains the grouping
and summary variables available in the dataset, which vary between
different datasets.

The example below shows how to access data stored in list columns
...
requesting all dataset information with
[`nomis_data_info()`](https://docs.ropensci.org/nomisr/reference/nomis_data_info.md),
...
`nomisr` also contains the
[`nomis_search()`](https://docs.ropensci.org/nomisr/reference/nomis_search.md)
function to search for datasets on particular topics.
[`nomis_search()`](https://docs.ropensci.org/nomisr/reference/nomis_search.md)
can be used to search in one or more of dataset names, descriptions,
keywords, content type and units. If using multiple parameters,
[`nomis_search()`](https://docs.ropensci.org/nomisr/reference/nomis_search.md)
will return information on all datasets that match one or more
parameters. Character vectors of strings can be used in searches, and
likewise
[`nomis_search()`](https://docs.ropensci.org/nomisr/reference/nomis_search.md)
will return information on datasets that match one or more queries. The
\* is used as a wildcard symbol.
[`nomis_search()`](https://docs.ropensci.org/nomisr/reference/nomis_search.md)
returns metadata in the same format as
[`nomis_data_info()`](https://docs.ropensci.org/nomisr/reference/nomis_data_info.md),
including using list-columns. The
[`nomis_content_type()`](https://docs.ropensci.org/nomisr/reference/nomis_content_type.md)
function can assist in identifying content type IDs for
[`nomis_search()`](https://docs.ropensci.org/nomisr/reference/nomis_search.md).
...
[`nomis_overview()`](https://docs.ropensci.org/nomisr/reference/nomis_overview.md)
has a `select` parameter that can be used to select only particular
elements of the overview to return.
...
Vast amounts of data are available through Nomis and so to avoid
overwhelming the API, it is good practice to query what concepts are
available, using
[`nomis_get_metadata()`](https://docs.ropensci.org/nomisr/reference/nomis_get_metadata.md).
While the other metadata functions can return concept metadata,
[`nomis_get_metadata()`](https://docs.ropensci.org/nomisr/reference/nomis_get_metadata.md)
provides greater flexibility and specificity over the returned metadata
than
[`nomis_overview()`](https://docs.ropensci.org/nomisr/reference/nomis_overview.md)
and
[`nomis_data_info()`](https://docs.ropensci.org/nomisr/reference/nomis_data_info.md).
...
for the “LC4408EW - Tenure bynumber of persons per bedroom in household
by household type” dataset.

#### Getting concepts

If provided with just a dataset ID,
[`nomis_get_metadata()`](https://docs.ropensci.org/nomisr/reference/nomis_get_metadata.md)
...
If provided with a concept name it returns the available values for that
concept. However, in some cases, espescially with the geography concept,
there are multiple options available, which Nomis labels types. In that
case
[`nomis_get_metadata()`](https://docs.ropensci.org/nomisr/reference/nomis_get_metadata.md)
...
We can now pass a generic “type” string to the `type` parameter in
[`nomis_get_metadata()`](https://docs.ropensci.org/nomisr/reference/nomis_get_metadata.md),
...
Passing a specific type to the `type` parameter, in this case “TYPE460”
for all post-2010 parliamentary constituencies, returns a tibble with
geographic codes for those specific constituencies, which can be used to
filter queries.
...
The vast majority (98% as of February 2018) of Nomis datasets include a
geographic variable.

### Downloading data

Using the information above, we can now query the latest data on bedroom
occupancy per household type in different NHS clinical commissioning
groups.
...
We can also query bedroom occupancy per household type in the
...
`nomisr` also allows for time series queries. The example below shows
how to retrieve the percentage of the workforce claiming Jobseekers
Allowance from January 2015 to January 2020, inclusive, for each region
of the UK, divided by male and female claimants, with an accompanying
graph.
